...
class AttentionMapVisualizer:
    def __init__(self, v, h_attn, w_attn, h_img, w_img):
        """
        초기화
        :param v: 뷰(View)의 수
        :param h_attn: 어텐션 맵의 높이 (예: 32)
        :param w_attn: 어텐션 맵의 너비 (예: 32)
        :param h_img: 원본 이미지의 높이 (예: 512)
        :param w_img: 원본 이미지의 너비 (예: 512)
        """
        self.V = v
        self.H_attn = h_attn
        self.W_attn = w_attn
        ...
        if h_img % h_attn != 0 or w_img % w_attn != 0:
            raise ValueError("이미지와 어텐션 맵의 크기가 정수배로 나누어 떨어지지 않습니다.")
            
        self.scale_factor_h = h_img // h_attn
        self.scale_factor_w = w_img // w_attn
        self.point_radius = max(2, self.scale_factor_h // 4) # 시각화 점의 반지름

    def _unravel_index(self, i):
        """
        Flatten된 *글로벌* 인덱스 i를 (v, y, x) 좌표로 변환합니다.
        """
        hw = self.H_attn * self.W_attn
        v = i // hw
        remainder = i % hw
        y = remainder // self.W_attn
        x = remainder % self.W_attn
        return v, y, x

    def _unravel_index_local(self, i):
        """
        Flatten된 *로컬* 인덱스 i (0 ~ H*W-1)를 (y, x) 좌표로 변환합니다.
        """
        y = i // self.W_attn
        x = i % self.W_attn
        return y, x

    def _get_positional_color(self, y, x):
        """
        어텐션 맵 좌표 (y, x)에 따라 고유하고 시인성이 좋은 (B, G, R) 색상을 생성합니다.
        HSV 색상 공간을 사용하여 y좌표(행)를 색상(Hue)에, x좌표(열)를 밝기(Value)에 매핑합니다.
        """
        # y-coordinate (row) to Hue (0-179 range in OpenCV)
        hue = int(179 * (y / (self.H_attn - 1)))
        
        # x-coordinate (column) to Value (brightness)
        # Use a bright range, e.g., 150-255, so no points are too dark
        value = int(150 + 105 * (x / (self.W_attn - 1)))
        
        # Keep Saturation at maximum (255) for vivid colors
        saturation = 255
        
        # Create the single-pixel HSV image
        hsv_color_np = np.uint8([[[hue, saturation, value]]])
        
        # Convert HSV to BGR (which OpenCV uses)
        bgr_color_np = cv2.cvtColor(hsv_color_np, cv2.COLOR_HSV2BGR)
        
        # Extract the (B, G, R) tuple and return it
        b, g, r = bgr_color_np[0, 0]
        return (int(b), int(g), int(r))

    def _get_scaled_coords(self, y, x):
        """
        어텐션 좌표 (y, x)를 원본 이미지 좌표 (y_img, x_img)로 스케일링합니다.
        패치 중앙에 위치하도록 스케일 값의 절반을 더합니다.
        """
        y_img = (y * self.scale_factor_h) + (self.scale_factor_h // 2)
        x_img = (x * self.scale_factor_w) + (self.scale_factor_w // 2)
        return y_img, x_img

    def _draw_point(self, image_canvas, v, y, x, color):
        """
        지정된 뷰(v)의 스케일링된 좌표 (y, x)에 색상 점을 그립니다.
        """
        y_img, x_img = self._get_scaled_coords(y, x)
        cv2.circle(
            image_canvas[v], 
            (x_img, y_img), 
    ...
    def visualize(self, attn_map, base_images_np, grid_size=16, query_view=0, per_view=False):
        """
        :param attn_map: (H_attn*W_attn, V*H_attn*W_attn) 크기의 PyTorch 텐서.
        :param base_images_np: (V, H_img, W_img, C) 크기의 NumPy 배열 (0-255, uint8, BGR)
        :param grid_size: 쿼리 그리드의 크기 (예: 16 -> 16x16 그리드)
        :param query_view: 쿼리 지점을 *그릴* 기준 뷰 인덱스
        :param per_view: True이면 각 키 뷰(key view)별로 argmax를 찾음, False이면 글로벌 argmax를 찾음.
        :return: (query_canvas, key_canvas) - NumPy 배열 튜플
        """
        if not isinstance(attn_map, torch.Tensor):
            raise TypeError("attn_map은 PyTorch 텐서여야 합니다.")
        if not isinstance(base_images_np, np.ndarray):
            raise TypeError("base_images_np는 NumPy 배열이어야 합니다.")
        
        # 1. 시각화를 위한 캔버스 준비
        query_canvas_np = base_images_np.copy()
        key_canvas_np = base_images_np.copy()
        
        hw_attn = self.H_attn * self.W_attn
        
        # 2. 쿼리 그리드 생성
        y_coords = np.linspace(0, self.H_attn - 1, grid_size, dtype=int)
        x_coords = np.linspace(0, self.W_attn - 1, grid_size, dtype=int)

        # 3. 각 쿼리 지점에 대해 반복
        for q_y, q_x in itertools.product(y_coords, x_coords):
            q_v = query_view
            
            # 쿼리의 고유 색상 가져오기
            query_color = self._get_positional_color(q_y, q_x)
            
            # (1) 쿼리 캔버스에 쿼리 지점 그리기
            self._draw_point(query_canvas_np, q_v, q_y, q_x, query_color)
            
            local_query_index = (q_y * self.W_attn) + q_x
            attn_row = attn_map[local_query_index] # (V*H*W,) tensor

            if not per_view:
                # --- 기존 방식: 글로벌 Argmax ---
                # 전체 뷰에서 가장 주목받는 단 하나의 키를 찾음
                key_index_1d = torch.argmax(attn_row)
                p_v, p_y, p_x = self._unravel_index(key_index_1d.item())
                
                # (3) 키 캔버스에 Max Point 그리기
                self._draw_point(key_canvas_np, p_v, p_y, p_x, query_color)
            else:
                # --- 새 방식: 뷰별 Argmax ---
                # 각 키 뷰(k_v)를 순회하며 해당 뷰 내에서의 max 값을 찾음
                for k_v in range(self.V):
                    # 이 쿼리에 대한 k_v 뷰의 어텐션 맵 슬라이스를 가져옴
                    
                    view_attn_slice = attn_row[k_v * hw_attn : (k_v + 1) * hw_attn] # (H*W,) tensor

                    # 이 뷰 *내에서* 가장 주목받는 키의 로컬 인덱스를 찾음
                    local_key_index_1d = torch.argmax(view_attn_slice)
                    
                    # 로컬 인덱스를 (y, x) 좌표로 변환
                    p_y, p_x = self._unravel_index_local(local_key_index_1d.item())
                    
                    # (3) 해당 키 뷰(k_v)의 캔버스에 Max Point 그리기
                    self._draw_point(key_canvas_np, k_v, p_y, p_x, query_color)

        return query_canvas_np, key_canvas_np
...
def main(nframe, cond_num, inference_view_range, 
    ...
    def unet_inference(batch, go_this_scene = True):
        ...
        setattr(pipeline.unet, 'cached', False)  # reset for next scene
    ...
    for idx, batch in enumerate(val_dataloader):
        ...
        if not go_this_scene:
            # just make random gaussian latent and skip
            # for the match random seed for all scenes
            _ = unet_inference(batch, go_this_scene)
            continue
            ...
            for unet_layer in caching_unet_attn_layers:
                ...
                for t in range(0, nframe):
                    ...
                    if Bc == 3:  # (kept from your original heuristic)
                        ...
                        attn_maps = sliced[1]  # [Head, Q, K], only pick conditional path

                        if config.unet_visualize.grid_query:
                            visualizer = AttentionMapVisualizer(
                                v=config.unet_visualize.nframe, 
                                h_attn=int(math.sqrt(Q // nframe)),
                                w_attn=int(math.sqrt(Q // nframe)), 
                                h_img=512,
                                w_img=512,
                            )
                            query_canvas, key_canvas = visualizer.visualize(
                                attn_maps.mean(dim=0),
                                images.squeeze(0).permute(0, 2, 3, 1).cpu().numpy() * 255,
                                grid_size=int(math.sqrt(Q // nframe)),
                                query_view=t,
                                per_view=config.unet_visualize.per_view,
                            )
                            ...
                            combined_img_grid_pil.save(os.path.join(sample_root, f"attn{unet_layer}_grid_query_frame{t}.png"))
                    ...
                    if config.unet_visualize.grid_query: continue
                    combined_img = save_image_jinhk(tgt_gt_image, images, t, x_coord, y_coord, score)
                    combined_img_q_on_gen = save_image_jinhk(images.squeeze()[t], images, t, x_coord, y_coord, score) if t >= cond_num else combined_img
                    stacked_images.append(combined_img)
                    stacked_images_q_on_gen.append(combined_img_q_on_gen)
                if config.unet_visualize.grid_query: continue
                stacked_images = concat_vertical(stacked_images)
                stacked_images_q_on_gen = concat_vertical(stacked_images_q_on_gen)
                stacked_images.save(os.path.join(sample_root, f"attn{unet_layer}.png"))
                stacked_images_q_on_gen.save(os.path.join(sample_root, f"attn{unet_layer}_q_on_gen.png"))
