...
def rigid_points_registration(pts1, pts2, conf):
    R, T, s = roma.rigid_points_registration(pts1.reshape(-1, 3), pts2.reshape(-1, 3),
                                             weights=conf.ravel(), compute_scaling=True)
    return s, R, T  # return un-scaled (R, T)


def RT_to_4x4(R, T, device):
...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def __init__(self, *args, optimize_pp=False, focal_break=20, **kwargs):
        ...
        if self.same_focals:
            ...
            self.im_focals = nn.ParameterList(torch.FloatTensor([self.focal_break * np.log(max(H, W))]) for H, W in self.imshapes)  # camera intrinsics
    ...
    def align_to_pts3d(self, pts3d):
        # we assume two predicted ply are well aligned
        # pts3d: (h,w,3) target pcd which need to be aligned to
        # view should be '0_1' '1_0'
        from .init_im_poses import geotrf
        s, R, T = rigid_points_registration(self.pred_i['0_1'], pts3d, conf=self.conf_i['0_1'])
        RT = RT_to_4x4(R * s, T, device=self.pred_j['0_1'].device)
        self.pred_j['0_1'] = geotrf(RT, self.pred_j['0_1'])

        s, R, T = rigid_points_registration(self.pred_j['1_0'], pts3d, conf=self.conf_j['1_0'])
        RT = RT_to_4x4(R * s, T, device=self.pred_i['1_0'].device)
        self.pred_i['1_0'] = geotrf(RT, self.pred_i['1_0'])

        pose0 = self.get_im_poses()[0] # c2w
        cam_pred0 = geotrf(pose0.inverse(), self.pred_j['0_1'].reshape(-1, 3)) # world coord to camera coord

        pose1 = self.get_im_poses()[1]  # c2w
        cam_pred1 = geotrf(pose1.inverse(), self.pred_i['1_0'].reshape(-1, 3))  # world coord to camera coord

        cam_pred = (cam_pred0 + cam_pred1) / 2

        self.im_depthmaps.requires_grad = False
        self.im_depthmaps[-1, :] = torch.log(cam_pred[:, -1])

        self.temp_pcd = (self.pred_j['0_1'] + self.pred_i['1_0']) / 2
