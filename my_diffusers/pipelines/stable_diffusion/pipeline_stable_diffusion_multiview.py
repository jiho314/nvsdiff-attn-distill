...
class StableDiffusionMultiViewPipeline(
    ...
    def __call__(
        ...
        if camera_embedding is None:
            ...
            add_inputs = torch.cat([masks, camera_embedding], dim=2)  # [b,f,c,h,w]
        coords = None
        if kwargs['config'].model_cfg.get("enable_depth", False):
            if kwargs['config'].model_cfg.get("priors3d", False):
                coords = get_3d_priors(kwargs['config'], kwargs["depth"], intrinsics, extrinsics,
                                       ...
                                       vae=kwargs['vae'], prior_type=kwargs['config'].model_cfg.get("prior_type", "3dpe"))
            else:
    ...
    def batch_call(
        ...
        # 2. Define call parameters
        if images.ndim == 4:
            assert images.shape[0] == nframe, "no batch dim, should be nframe"
        ...
        if no_camera_emb:
            ...
            extrinsics = einops.rearrange(extrinsics, "b f c1 c2 -> (b f) c1 c2", f=nframe)
            intrinsics = einops.rearrange(intrinsics, "b f c1 c2 -> (b f) c1 c2", f=nframe)
            camera_embedding = get_camera_embedding(intrinsics, extrinsics,
                                                    batch_size, nframe, height, width,
                                                    config=kwargs['config']).to(device=device)
        ### build masks (random visible frames), valid:0, mask:1 ###
        ...
        image_latents = self._encode_vae_image(image=images, generator=generator)
        images = einops.rearrange(images, '(b f) c h w -> b f c h w', b=batch_size, f=nframe)
        image_latents = einops.rearrange(image_latents, '(b f) c h w -> b f c h w', b=batch_size, f=nframe)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # compute the previous noisy sample x_t -> x_t-1
                noise_pred = noise_pred.reshape(batch_size, nframe, *noise_pred.shape[1:])
                latents = scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=False)[0]
                ### blending ###
                latents[:, :cond_num] = image_latents[:, :cond_num]
