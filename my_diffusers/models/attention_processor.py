...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        is_cross_attn = encoder_hidden_states is not None  # bug fixed!
        ...
        if image_rotary_emb is not None and query.shape[2] == key.shape[2]:
            if full_attention:
                image_rotary_emb_ = image_rotary_emb[query.shape[2] // nframe]
                image_rotary_emb_ = einops.repeat(image_rotary_emb_, "b nh hw c c1 c2 -> b nh (f hw) c c1 c2", f=nframe)
            else:
                image_rotary_emb_ = image_rotary_emb[query.shape[2]]
            query, key = apply_rope(query, key, image_rotary_emb_)

        # the output of sdp = (batch, num_heads, seq_len, head_dim)
        # TODO: add support for attn.scale when we move to Torch 2.1
        # multiply a constant to key features
        if full_attention and nframe > 28 and cond_num is not None and key_rescale is not None:
