                ...
                if do_attn_distill:
                    with torch.autocast(device_type="cuda", dtype = weight_dtype):
                        ...
                        for idx, (unet_layer, vggt_layer) in zip(distill_pairs_idx, distill_pairs):
                            ...
                            # pred logit head
                            logit_head_kwargs = {'num_view': F, 'num_view_query': F}
                            pred_attn_logit = unet.unet_logit_head[str(unet_layer)](pred_attn_logit, **logit_head_kwargs) # B Head Q(FHW) K(FHW)
                            ...
                            # logit head
                            logit_head_kwargs = {'num_view': F, 'num_view_query': F}
                            gt_attn_logit = unet.vggt_logit_head[str(vggt_layer)](gt_attn_logit, **logit_head_kwargs)
                            ...
                            if len(key_idx) > 0 :
                                # 1) slice
                                ref_pred, f1, f2 = slice_attnmap(pred_attn_logit, ref_query_idx, key_idx, exclude_selfattn="self" not in key_list)
                                ref_gt, _, _ = slice_attnmap(gt_attn_logit, ref_query_idx, key_idx, exclude_selfattn="self" not in key_list)
                                ...
                                # # 2) logit head : 10/27: logit head done before 3. loss compute #TODO remove
                            ...
                            if len(key_idx) > 0 :
                                # 1) slice
                                tgt_pred, f1, f2 = slice_attnmap(pred_attn_logit, tgt_query_idx, key_idx, exclude_selfattn="self" not in key_list)
                                tgt_gt, _, _ = slice_attnmap(gt_attn_logit, tgt_query_idx, key_idx, exclude_selfattn="self" not in key_list)
                                if mask_per_view is not None:
